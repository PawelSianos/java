/**
 * Created by Paweł on 2017-08-26.
 */
//You found two items in a treasure chest! The first item weighs weight1 and is worth value1,
//and the second item weighs weight2 and is worth value2.
//What is the total maximum value of the items you can take with you,
//assuming that your max weight capacity is maxW and you can't come back for the items later?
//Note that there are only two items and you can't bring more than one item of each type,
//i.e. you can't take two first items or two second items.
public class Main {
    public static void main(String... args){
        int value1 = 10;
        int value2 = 6;
        int weight1 = 5;
        int weight2 = 4;
        int maxWeight = 10;
        System.out.println(knapsackLight(value1, weight1,value2, weight2,maxWeight));
    }
    static int knapsackLight(int value1, int weight1, int value2, int weight2, int maxW) {

        int bigggerValue, biggerWeight, lesserValue, lesserWeight;

        if(value1 >= value2){
            bigggerValue = value1;
            biggerWeight = weight1;
            lesserValue = value2;
            lesserWeight = weight2;
        }else{
            bigggerValue = value2;
            biggerWeight = weight2;
            lesserValue = value1;
            lesserWeight = weight1;
        }
        if(maxW >= weight1+weight2)
            return value1+value2;
        else if(maxW >=biggerWeight)
            return bigggerValue;
        else if(maxW >=lesserWeight)
            return lesserValue;
        else
            return 0;
    }

}
